...
const updatedDataObj = dataObj.map((el) => ({ ...el, slug: slugify(el.productName, { lower: true }) }));
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/' || pathname === '/overview') {
    ...
    const cardsHtml = updatedDataObj.map((el) => replaceTemplate(templateCard, el)).join('');
    ...
    res.writeHead(200, { 'Content-type': 'text/html' });
    const product = updatedDataObj.find((el) => el.slug === pathname.split('/')[2]);
